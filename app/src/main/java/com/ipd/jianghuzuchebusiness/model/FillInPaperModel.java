...
public class FillInPaperModel<T> extends BaseModel {
    ...
    public void getGetCarCommit(Context context, TreeMap<String, String> map, boolean isDialog, boolean cancelable,
        ...
        paramSubscribe(context, Api.getApiService().getGetCarCommit(map), observerListener, transformer, isDialog, cancelable);
    }

    public void getReturnCarCommit(Context context, TreeMap<String, String> map, boolean isDialog, boolean cancelable,
                                   ObservableTransformer<T, T> transformer, ObserverResponseListener observerListener) {

        //当不需要指定是否由dialog时，可以调用这个方法
        //        subscribe(context, Api.getApiService().login(map), observerListener);
        paramSubscribe(context, Api.getApiService().getReturnCarCommit(map), observerListener, transformer, isDialog, cancelable);
    }
