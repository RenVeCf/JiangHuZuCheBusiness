 ...
 * Time ： 2018/8/26.
 */
public class SelectOpeningBankModel<T> extends BaseModel {

    public void getSelectOpeningBank(Context context, boolean isDialog, boolean cancelable,
                                     ObservableTransformer<T, T> transformer, ObserverResponseListener observerListener) {

        //当不需要指定是否由dialog时，可以调用这个方法
        //        subscribe(context, Api.getApiService().login(map), observerListener);
        nullParamSubscribe(context, Api.getApiService().getSelectOpeningBank(), observerListener, isDialog, cancelable);
    }
